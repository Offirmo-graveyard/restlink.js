...
function(_, when, Request, BaseServerAdapter, EE, SerializationUtils) {
	...
	function send_express_response_on_throw(express_response, e) {
		// this code must be ultra-safe !

		express_response.status(500);
		express_response.set('Content-Type', 'text/plain');
		...
		console.log("< [express adapter] response sent (on exception).");
	}

	// convert restlink response to HTTP response
	function format_and_send_express_response(http_res, restlink_res) {
		...
		try {
			...
			console.log("< [express adapter] response sent.");
		}
		catch (e) {
			send_express_response_on_throw(http_res, e);
		}
	}
	...
	// on incoming request
	function express_callback(express_request, express_response, next) {
		...
		try {
			console.log("> [express adapter] request received...");
			...
			http_request.on('end', function () {
				...
				try {
					...
					promise.then( function(restlink_response) {
						format_and_send_express_response(http_response, restlink_response);
					},
					function(e){
					...
					send_express_response_on_throw(http_response, e);
				}
			});
		}
		catch (e) {
	...
	methods.startup = function(core) {
		console.log("* starting Express restlink adapter...");

		// call parent
		Parent.methods.startup.apply(this, arguments);
	};
	methods.shutdown = function() {
		console.log("* stopping Express restlink adapter...");
	...
	var DefinedClass = function RestlinkServerExpressAdapter(options) {
