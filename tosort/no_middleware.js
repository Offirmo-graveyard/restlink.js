/* A default RestLink request handler middleware.
 * Always answer with an error.
 * This middleware is automatically used by a server if no other MW were provided.
 */
...
function(_, RestlinkMiddlewareBase) {
	...
	function processing_function(context, request, response, next) {
		response.set_to_not_implemented("Server is misconfigured. Please add middlewares to handle requests !");
		response.send();
	}
